...
# 指针类型
DLArrayHandle = ctypes.POINTER(DLArray)   # 返回值是一个类型对象，
...
def empty(shape, ctx=cpu(0)):
    ...
    :return:
    """
    shape = c_array(ctypes.c_int64, shape)  # 将tuple转成了c中的数组类型
    ndim = ctypes.c_int(len(shape))  # 获取一个int类型的dim
